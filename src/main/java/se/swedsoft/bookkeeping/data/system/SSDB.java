...
public class SSDB {    private static final Logger LOG = LoggerFactory.getLogger(SSDB.class);
    ...
    private List<SSPurchaseOrderRow> getPurchaseOrderRowsV2(Integer iPurchaseOrderId) throws SQLException {
        ...
        iStatement.setObject(1, iPurchaseOrderId);
        ResultSet iResultSet = iStatement.executeQuery();
        while (iResultSet.next()) {
            SSPurchaseOrderRow iRow = new SSPurchaseOrderRow();
            iRow.setProductNr(iResultSet.getString("product_nr"));
            iRow.setDescription(iResultSet.getString("description"));
            iRow.setSupplierArticleNr(iResultSet.getString("supplier_article_nr"));
            iRow.setUnitPrice(iResultSet.getBigDecimal("unitprice"));
            iRow.setQuantity((Integer) iResultSet.getObject("quantity"));
    ...
    private void replacePurchaseOrderRowsV2(Integer iPurchaseOrderId, SSPurchaseOrder iPurchaseOrder)
            throws SQLException {
        PreparedStatement iDelete = iConnection.prepareStatement(
                "DELETE FROM tbl_purchaseorder_row WHERE purchaseorder_id=?");
        iDelete.setObject(1, iPurchaseOrderId);
        iDelete.executeUpdate();
        iDelete.close();
        ...
        for (SSPurchaseOrderRow iRow : iPurchaseOrder.getRows()) {
            ...
            iInsert.setObject(1, iPurchaseOrderId);
            iInsert.setObject(2, iRow.getProductNr());
            iInsert.setObject(3, iRow.getDescription());
            iInsert.setObject(4, iRow.getSupplierArticleNr());
            iInsert.setObject(5, iRow.getUnitPrice());
            iInsert.setObject(6, iRow.getQuantity());
            iInsert.setObject(7, iRow.getUnit() == null ? null : iRow.getUnit().getName());
            iInsert.setObject(8, iRow.getAccountNr());
            iInsert.executeUpdate();
            iInsert.close();
        }
    ...
    private Map<SSDefaultAccount, Integer> getPurchaseOrderDefaultAccountsV2(Integer iPurchaseOrderId)
            throws SQLException {
        Map<SSDefaultAccount, Integer> iMap = new HashMap<>();
        PreparedStatement iStatement = iConnection.prepareStatement(
                "SELECT account_type,account_nr FROM tbl_purchaseorder_account WHERE purchaseorder_id=?");
        iStatement.setObject(1, iPurchaseOrderId);
        ResultSet iResultSet = iStatement.executeQuery();
        while (iResultSet.next()) {
            String iType = iResultSet.getString("account_type");
            Integer iAccountNr = (Integer) iResultSet.getObject("account_nr");
            if (iType == null || iAccountNr == null) {
                continue;
            }
            try {
                iMap.put(SSDefaultAccount.valueOf(iType), iAccountNr);
    ...
    private void replacePurchaseOrderDefaultAccountsV2(Integer iPurchaseOrderId, SSPurchaseOrder iPurchaseOrder)
            throws SQLException {
        PreparedStatement iDelete = iConnection.prepareStatement(
        ...
        Map<SSDefaultAccount, Integer> iDefaults = iPurchaseOrder.getDefaultAccounts();
        if (iDefaults == null || iDefaults.isEmpty()) {
            return;
        }

        for (Map.Entry<SSDefaultAccount, Integer> iEntry : iDefaults.entrySet()) {
            if (iEntry.getKey() == null || iEntry.getValue() == null) {
                continue;
            }
            PreparedStatement iInsert = iConnection.prepareStatement(
                    "INSERT INTO tbl_purchaseorder_account(purchaseorder_id,account_type,account_nr) VALUES(?,?,?)");
            iInsert.setObject(1, iPurchaseOrderId);
            iInsert.setObject(2, iEntry.getKey().name());
            iInsert.setObject(3, iEntry.getValue());
            iInsert.executeUpdate();
            iInsert.close();
        }
    }

    private Integer getPurchaseOrderIdV2(Integer iPurchaseOrderNumber, Integer iCompanyId)
            throws SQLException {
        if (iPurchaseOrderNumber == null || iCompanyId == null) {
            return null;
        }
        PreparedStatement iStatement = iConnection.prepareStatement(
                "SELECT id FROM tbl_purchaseorder WHERE number=? AND companyid=?");
        iStatement.setObject(1, iPurchaseOrderNumber);
        iStatement.setObject(2, iCompanyId);
        ResultSet iResultSet = iStatement.executeQuery();
        try {
            if (iResultSet.next()) {
    ...
    private int bindPurchaseOrderColumnsV2(
            PreparedStatement iStatement,
            int iIndex,
            SSPurchaseOrder iPurchaseOrder) throws SQLException {
        iStatement.setObject(iIndex++, iPurchaseOrder.getInvoiceNr());
        bindLocalDateV2(iStatement, iIndex++, iPurchaseOrder.getLocalDate());
        iStatement.setObject(iIndex++, iPurchaseOrder.getSupplierNr());
        iStatement.setObject(iIndex++, iPurchaseOrder.getSupplierName());
        bindLocalDateV2(iStatement, iIndex++, iPurchaseOrder.getLocalEstimatedDelivery());
        iStatement.setObject(iIndex++, iPurchaseOrder.getPaymentTerm() == null
                ? null
                : iPurchaseOrder.getPaymentTerm().getName());
        iStatement.setObject(iIndex++, iPurchaseOrder.getDeliveryTerm() == null
                ? null
                : iPurchaseOrder.getDeliveryTerm().getName());
        iStatement.setObject(iIndex++, iPurchaseOrder.getDeliveryWay() == null
                ? null
                : iPurchaseOrder.getDeliveryWay().getName());
        iStatement.setObject(iIndex++, iPurchaseOrder.getOurContact());
        iStatement.setObject(iIndex++, iPurchaseOrder.getYourContact());
        iStatement.setObject(iIndex++, getCurrencyCodeV2(iPurchaseOrder.getCurrency()));
        iStatement.setObject(iIndex++, iPurchaseOrder.getCurrencyRate());
        iStatement.setObject(iIndex++, iPurchaseOrder.getText());
        iStatement.setObject(iIndex++, iPurchaseOrder.isPrinted());
        iStatement.setObject(iIndex++, iPurchaseOrder.isStockInfluencing());
        iIndex = bindAddressV2(iStatement, iIndex, iPurchaseOrder.getDeliveryAddress());
        return bindAddressV2(iStatement, iIndex, iPurchaseOrder.getSupplierAddress());
    }

    private SSPurchaseOrder mapPurchaseOrderV2(ResultSet iResultSet) throws SQLException {
        SSPurchaseOrder iPurchaseOrder = new SSPurchaseOrder();

        iPurchaseOrder.setNumber((Integer) iResultSet.getObject("number"));
        iPurchaseOrder.setInvoiceNr((Integer) iResultSet.getObject("invoice_nr"));

        java.sql.Date iDate = iResultSet.getDate("vdate");
        if (iDate != null) {
            iPurchaseOrder.setLocalDate(iDate.toLocalDate());
        }

        iPurchaseOrder.setSupplierNr(iResultSet.getString("supplier_nr"));
        iPurchaseOrder.setSupplierName(iResultSet.getString("supplier_name"));

        java.sql.Date iEstimatedDelivery = iResultSet.getDate("estimated_delivery");
        if (iEstimatedDelivery != null) {
            iPurchaseOrder.setLocalEstimatedDelivery(iEstimatedDelivery.toLocalDate());
        }

        String iPaymentTerm = iResultSet.getString("payment_term");
        if (iPaymentTerm != null) {
            iPurchaseOrder.setPaymentTerm(new SSPaymentTerm(iPaymentTerm, iPaymentTerm));
        }

        String iDeliveryTerm = iResultSet.getString("delivery_term");
        if (iDeliveryTerm != null) {
            iPurchaseOrder.setDeliveryTerm(new SSDeliveryTerm(iDeliveryTerm, iDeliveryTerm));
        }

        String iDeliveryWay = iResultSet.getString("delivery_way");
        if (iDeliveryWay != null) {
            iPurchaseOrder.setDeliveryWay(new SSDeliveryWay(iDeliveryWay, iDeliveryWay));
        }

        iPurchaseOrder.setOurContact(iResultSet.getString("our_contact"));
        iPurchaseOrder.setYourContact(iResultSet.getString("your_contact"));

        String iCurrencyCode = iResultSet.getString("currency_code");
        if (iCurrencyCode != null) {
            iPurchaseOrder.setCurrency(new SSCurrency(iCurrencyCode, iCurrencyCode));
        }

        iPurchaseOrder.setCurrencyRate(iResultSet.getBigDecimal("currency_rate"));
        iPurchaseOrder.setText(iResultSet.getString("sale_text"));
        iPurchaseOrder.setPrinted(iResultSet.getBoolean("printed"));
        iPurchaseOrder.setStockInfluencing(iResultSet.getBoolean("stock_influencing"));
        iPurchaseOrder.setDeliveryAddress(mapAddressV2(iResultSet, "del_addr"));
        iPurchaseOrder.setSupplierAddress(mapAddressV2(iResultSet, "supp_addr"));

        Integer iPurchaseOrderId = iResultSet.getInt("id");
        iPurchaseOrder.getRows().clear();
        iPurchaseOrder.getRows().addAll(getPurchaseOrderRowsV2(iPurchaseOrderId));
        iPurchaseOrder.setDefaultAccounts(getPurchaseOrderDefaultAccountsV2(iPurchaseOrderId));
    ...
    public List<SSOrder> getOrders() {
        ...
        if (!ensureSchemaV2ForDomain("Order")) {
            return iOrders;
        }
        try {
            Integer iMax = -1;
    ...
    public Optional<SSOrder> getOrder(SSOrder pOrder) {
        ...
        try {
            ...
            if (iResultSet.next()) {
                SSOrder iOrder = mapOrderV2(iResultSet);
    ...
    public List<SSInvoice> getInvoices() {
        ...
        if (!ensureSchemaV2ForDomain("Invoice")) {
            return iInvoices;
        }
        try {
            Integer iMax = -1;
    ...
    public Optional<SSInvoice> getInvoice(SSInvoice pInvoice) {
        ...
        try {
            ...
            if (iResultSet.next()) {
                SSInvoice iInvoice = mapInvoiceV2(iResultSet);
    ...
    public List<SSInpayment> getInpayments() {
        ...
        if (!ensureSchemaV2ForDomain("Inpayment")) {
            return iInpayments;
        }
        try {
            Integer iMax = -1;
    ...
    public Optional<SSInpayment> getInpayment(SSInpayment pInpayment) {
        ...
        try {
            ...
            if (iResultSet.next()) {
                SSInpayment iInpayment = mapInpaymentV2(iResultSet);
    ...
    public List<SSOutpayment> getOutpayments() {
        ...
        if (!ensureSchemaV2ForDomain("Outpayment")) {
            return iOutpayments;
        }
        try {
            Integer iMax = -1;
    ...
    public Optional<SSOutpayment> getOutpayment(SSOutpayment pOutpayment) {
        ...
        try {
            ...
            if (iResultSet.next()) {
                SSOutpayment iOutpayment = mapOutpaymentV2(iResultSet);
    ...
    public List<SSPurchaseOrder> getPurchaseOrders() {
        ...
        if (!ensureSchemaV2ForDomain("PurchaseOrder")) {
            return iPurchaseOrders;
        }
        try {
            Integer iMax = -1;
            ...
            while (true) {
                ...
                while (iResultSet.next()) {
                    iMax = iResultSet.getInt(1);
                    iPurchaseOrders.add(mapPurchaseOrderV2(iResultSet));
                    i++;
                }
                if (i != 1024) {
                    break;
    ...
    public Optional<SSPurchaseOrder> getPurchaseOrder(SSPurchaseOrder pPurchaseOrder) {
        ...
        try {
            ...
            if (iResultSet.next()) {
                SSPurchaseOrder iPurchaseOrder = mapPurchaseOrderV2(iResultSet);
    ...
    public void addPurchaseOrder(SSPurchaseOrder iPurchaseOrder) {
        ...
        try {
            ...
            iStatement = iConnection.prepareStatement(
                    ...
                    Statement.RETURN_GENERATED_KEYS);
            int i = 1;
            iStatement.setObject(i++, iPurchaseOrder.getNumber());
            iStatement.setObject(i++, iCurrentCompany.getId());
            bindPurchaseOrderColumnsV2(iStatement, i, iPurchaseOrder);
            iStatement.executeUpdate();

            try (ResultSet iKeys = iStatement.getGeneratedKeys()) {
                if (iKeys.next()) {
                    iPurchaseOrderId = iKeys.getInt(1);
                }
            }
            if (iPurchaseOrderId == null) {
                iPurchaseOrderId = getPurchaseOrderIdV2(iPurchaseOrder.getNumber(), iCurrentCompany.getId());
            }
            if (iPurchaseOrderId != null) {
                replacePurchaseOrderRowsV2(iPurchaseOrderId, iPurchaseOrder);
                replacePurchaseOrderDefaultAccountsV2(iPurchaseOrderId, iPurchaseOrder);
            }
    ...
    public void updatePurchaseOrder(SSPurchaseOrder iPurchaseOrder) {
        ...
        try {
            ...
            int i = bindPurchaseOrderColumnsV2(iStatement, 1, iPurchaseOrder);
            iStatement.setObject(i++, iPurchaseOrder.getNumber());
            iStatement.setObject(i, iCurrentCompany.getId());
            iStatement.executeUpdate();
    ...
    public void deletePurchaseOrder(SSPurchaseOrder iPurchaseOrder) {
        ...
        try {
            Integer iPurchaseOrderId = getPurchaseOrderIdV2(iPurchaseOrder.getNumber(), iCurrentCompany.getId());
            if (iPurchaseOrderId != null) {
                PreparedStatement iDeleteRows = iConnection.prepareStatement(
                        "DELETE FROM tbl_purchaseorder_row WHERE purchaseorder_id=?");
                iDeleteRows.setObject(1, iPurchaseOrderId);
                iDeleteRows.executeUpdate();
                iDeleteRows.close();

                PreparedStatement iDeleteDefaults = iConnection.prepareStatement(
                        "DELETE FROM tbl_purchaseorder_account WHERE purchaseorder_id=?");
                iDeleteDefaults.setObject(1, iPurchaseOrderId);
                iDeleteDefaults.executeUpdate();
                iDeleteDefaults.close();
            }
    ...
    private List<SSSupplierInvoiceRow> getSupplierInvoiceRowsV2(Integer iSupplierInvoiceId) throws SQLException {
        List<SSSupplierInvoiceRow> iRows = new LinkedList<>();
        PreparedStatement iStatement = iConnection.prepareStatement(
                "SELECT * FROM tbl_supplierinvoice_row WHERE supplierinvoice_id=? ORDER BY id");
        iStatement.setObject(1, iSupplierInvoiceId);
        ResultSet iResultSet = iStatement.executeQuery();
        while (iResultSet.next()) {
            SSSupplierInvoiceRow iRow = new SSSupplierInvoiceRow();
            iRow.setProductNr(iResultSet.getString("product_nr"));
    ...
    private void replaceSupplierInvoiceRowsV2(Integer iSupplierInvoiceId, SSSupplierInvoice iSupplierInvoice)
            throws SQLException {
        PreparedStatement iDelete = iConnection.prepareStatement(
                "DELETE FROM tbl_supplierinvoice_row WHERE supplierinvoice_id=?");
        iDelete.setObject(1, iSupplierInvoiceId);
        iDelete.executeUpdate();
        iDelete.close();
        ...
        for (SSSupplierInvoiceRow iRow : iSupplierInvoice.getRows()) {
            ...
            iInsert.setObject(1, iSupplierInvoiceId);
            iInsert.setObject(2, iRow.getProductNr());
            iInsert.setObject(3, iRow.getDescription());
            iInsert.setObject(4, iRow.getUnitprice());
            iInsert.setObject(5, iRow.getQuantity());
    ...
    private Integer getSupplierInvoiceIdV2(Integer iSupplierInvoiceNumber, Integer iCompanyId)
            throws SQLException {
        if (iSupplierInvoiceNumber == null || iCompanyId == null) {
            return null;
        }
        PreparedStatement iStatement = iConnection.prepareStatement(
                "SELECT id FROM tbl_supplierinvoice WHERE number=? AND companyid=?");
        iStatement.setObject(1, iSupplierInvoiceNumber);
        iStatement.setObject(2, iCompanyId);
        ResultSet iResultSet = iStatement.executeQuery();
        try {
            if (iResultSet.next()) {
    ...
    private int bindSupplierInvoiceColumnsV2(
            PreparedStatement iStatement,
            int iIndex,
            SSSupplierInvoice iSupplierInvoice) throws SQLException {
        bindLocalDateV2(iStatement, iIndex++, iSupplierInvoice.getLocalDate());
        bindLocalDateV2(iStatement, iIndex++, iSupplierInvoice.getLocalDueDate());
        iStatement.setObject(iIndex++, iSupplierInvoice.getSupplierNr());
        iStatement.setObject(iIndex++, iSupplierInvoice.getSupplierName());
        iStatement.setObject(iIndex++, iSupplierInvoice.getReferencenumber());
        iStatement.setObject(iIndex++, getCurrencyCodeV2(iSupplierInvoice.getCurrency()));
        iStatement.setObject(iIndex++, iSupplierInvoice.getCurrencyRate());
        // SSSupplierInvoice currently has no public payment-term getter.
        iStatement.setObject(iIndex++, null);
        iStatement.setObject(iIndex++, iSupplierInvoice.getTaxSum());
        iStatement.setObject(iIndex++, iSupplierInvoice.getRoundingSum());
        iStatement.setObject(iIndex++, iSupplierInvoice.isEntered());
        iStatement.setObject(iIndex++, iSupplierInvoice.isStockInfluencing());
        iStatement.setObject(iIndex++, iSupplierInvoice.isBGCEntered());
        iStatement.setObject(iIndex++, getVoucherIdByNumberV2(iSupplierInvoice.getVoucher()));
        iStatement.setObject(iIndex++, getVoucherIdByNumberV2(iSupplierInvoice.getCorrection()));
        return iIndex;
    }

    private SSSupplierInvoice mapSupplierInvoiceV2(ResultSet iResultSet) throws SQLException {
        SSSupplierInvoice iSupplierInvoice = new SSSupplierInvoice();
        iSupplierInvoice.setNumber((Integer) iResultSet.getObject("number"));

        java.sql.Date iDate = iResultSet.getDate("vdate");
        if (iDate != null) {
            iSupplierInvoice.setLocalDate(iDate.toLocalDate());
        }

        java.sql.Date iDueDate = iResultSet.getDate("due_date");
        if (iDueDate != null) {
            iSupplierInvoice.setLocalDueDate(iDueDate.toLocalDate());
        }

        iSupplierInvoice.setSupplierNr(iResultSet.getString("supplier_nr"));
        iSupplierInvoice.setSupplierName(iResultSet.getString("supplier_name"));
        iSupplierInvoice.setReferencenumber(iResultSet.getString("reference_number"));

        String iCurrencyCode = iResultSet.getString("currency_code");
        if (iCurrencyCode != null) {
            iSupplierInvoice.setCurrency(new SSCurrency(iCurrencyCode, iCurrencyCode));
        }

        String iPaymentTerm = iResultSet.getString("payment_term");
        if (iPaymentTerm != null) {
            iSupplierInvoice.setPaymentTerm(new SSPaymentTerm(iPaymentTerm, iPaymentTerm));
        }

        iSupplierInvoice.setCurrencyRate(iResultSet.getBigDecimal("currency_rate"));
        iSupplierInvoice.setTaxSum(iResultSet.getBigDecimal("tax_sum"));
        iSupplierInvoice.setRoundingSum(iResultSet.getBigDecimal("rounding_sum"));
        iSupplierInvoice.setEntered(iResultSet.getBoolean("entered"));
        iSupplierInvoice.setStockInfluencing(iResultSet.getBoolean("stock_influencing"));
        iSupplierInvoice.setBGCEntered(iResultSet.getBoolean("bgc_entered"));

        Integer iVoucherId = (Integer) iResultSet.getObject("voucher_id");
        Integer iVoucherNumber = getVoucherNumberForIdV2(iVoucherId);
        if (iVoucherNumber != null) {
            iSupplierInvoice.setVoucher(new SSVoucher(iVoucherNumber));
        }

        Integer iCorrectionVoucherId = (Integer) iResultSet.getObject("correction_voucher_id");
        Integer iCorrectionVoucherNumber = getVoucherNumberForIdV2(iCorrectionVoucherId);
        if (iCorrectionVoucherNumber != null) {
            iSupplierInvoice.setCorrection(new SSVoucher(iCorrectionVoucherNumber));
        }

        iSupplierInvoice.getRows().clear();
        iSupplierInvoice.getRows().addAll(getSupplierInvoiceRowsV2(iResultSet.getInt("id")));
    ...
    public List<SSSupplierCreditInvoice> getSupplierCreditInvoices() {
        ...
        if (!ensureSchemaV2ForDomain("SupplierCreditInvoice")) {
            return iSupplierCreditInvoices;
        }
        try {
            Integer iMax = -1;
    ...
    public Optional<SSSupplierCreditInvoice> getSupplierCreditInvoice(SSSupplierCreditInvoice pSupplierCreditInvoice) {
        ...
        try {
            ...
            if (iResultSet.next()) {
                SSSupplierCreditInvoice iSupplierCreditInvoice = mapSupplierCreditInvoiceV2(iResultSet);
    ...
    private List<SSInventoryRow> getInventoryRowsV2(Integer iInventoryId) throws SQLException {
        List<SSInventoryRow> iRows = new LinkedList<>();
        PreparedStatement iStatement = iConnection.prepareStatement(
                "SELECT * FROM tbl_inventory_row WHERE inventory_id=? ORDER BY id");
        iStatement.setObject(1, iInventoryId);
        ResultSet iResultSet = iStatement.executeQuery();
        while (iResultSet.next()) {
            SSInventoryRow iRow = new SSInventoryRow();
            iRow.setProductNr(iResultSet.getString("product_nr"));
            iRow.setStockQuantity((Integer) iResultSet.getObject("quantity"));
            iRow.setChange((Integer) iResultSet.getObject("change_qty"));
            iRows.add(iRow);
        }
        iResultSet.close();
    ...
    private void replaceInventoryRowsV2(Integer iInventoryId, SSInventory iInventory) throws SQLException {
        PreparedStatement iDelete = iConnection.prepareStatement(
                "DELETE FROM tbl_inventory_row WHERE inventory_id=?");
        iDelete.setObject(1, iInventoryId);
        iDelete.executeUpdate();
        iDelete.close();
        ...
        for (SSInventoryRow iRow : iInventory.getRows()) {
            ...
            iInsert.setObject(1, iInventoryId);
            iInsert.setObject(2, iRow.getProductNr());
            iInsert.setObject(3, iRow.getStockQuantity());
            iInsert.setObject(4, iRow.getChange());
            iInsert.executeUpdate();
            iInsert.close();
        }
    }

    private Integer getInventoryIdV2(Integer iInventoryNumber, Integer iCompanyId) throws SQLException {
        if (iInventoryNumber == null || iCompanyId == null) {
            return null;
        }
        PreparedStatement iStatement = iConnection.prepareStatement(
                "SELECT id FROM tbl_inventory WHERE number=? AND companyid=?");
        iStatement.setObject(1, iInventoryNumber);
        iStatement.setObject(2, iCompanyId);
        ResultSet iResultSet = iStatement.executeQuery();
        try {
            if (iResultSet.next()) {
    ...
    private SSInventory mapInventoryV2(ResultSet iResultSet) throws SQLException {
        SSInventory iInventory = new SSInventory();
        iInventory.setNumber((Integer) iResultSet.getObject("number"));

        java.sql.Date iDate = iResultSet.getDate("vdate");
        if (iDate != null) {
            iInventory.setLocalDate(iDate.toLocalDate());
        }

        iInventory.setText(iResultSet.getString("itext"));
        iInventory.getRows().clear();
        iInventory.getRows().addAll(getInventoryRowsV2(iResultSet.getInt("id")));
        return iInventory;
    }
    ...
    private List<SSOutdeliveryRow> getOutdeliveryRowsV2(Integer iOutdeliveryId) throws SQLException {
        List<SSOutdeliveryRow> iRows = new LinkedList<>();
        PreparedStatement iStatement = iConnection.prepareStatement(
                "SELECT * FROM tbl_outdelivery_row WHERE outdelivery_id=? ORDER BY id");
        iStatement.setObject(1, iOutdeliveryId);
        ResultSet iResultSet = iStatement.executeQuery();
        while (iResultSet.next()) {
            SSOutdeliveryRow iRow = new SSOutdeliveryRow();
            iRow.setProductNr(iResultSet.getString("product_nr"));
            iRow.setChange((Integer) iResultSet.getObject("change_qty"));
            iRows.add(iRow);
        }
    ...
    private void replaceOutdeliveryRowsV2(Integer iOutdeliveryId, SSOutdelivery iOutdelivery) throws SQLException {
        PreparedStatement iDelete = iConnection.prepareStatement(
                "DELETE FROM tbl_outdelivery_row WHERE outdelivery_id=?");
        iDelete.setObject(1, iOutdeliveryId);
        iDelete.executeUpdate();
        iDelete.close();
        ...
        for (SSOutdeliveryRow iRow : iOutdelivery.getRows()) {
            ...
            iInsert.setObject(1, iOutdeliveryId);
            iInsert.setObject(2, iRow.getProductNr());
            iInsert.setObject(3, iRow.getChange());
            iInsert.executeUpdate();
            iInsert.close();
    ...
    private Integer getOutdeliveryIdV2(Integer iOutdeliveryNumber, Integer iCompanyId) throws SQLException {
        if (iOutdeliveryNumber == null || iCompanyId == null) {
            return null;
        }
        PreparedStatement iStatement = iConnection.prepareStatement(
                "SELECT id FROM tbl_outdelivery WHERE number=? AND companyid=?");
        iStatement.setObject(1, iOutdeliveryNumber);
        iStatement.setObject(2, iCompanyId);
        ResultSet iResultSet = iStatement.executeQuery();
        try {
            if (iResultSet.next()) {
    ...
    private SSOutdelivery mapOutdeliveryV2(ResultSet iResultSet) throws SQLException {
        SSOutdelivery iOutdelivery = new SSOutdelivery();
        iOutdelivery.setNumber((Integer) iResultSet.getObject("number"));

        java.sql.Date iDate = iResultSet.getDate("vdate");
        if (iDate != null) {
            iOutdelivery.setLocalDate(iDate.toLocalDate());
        }

        iOutdelivery.setText(iResultSet.getString("itext"));
        iOutdelivery.getRows().clear();
        iOutdelivery.getRows().addAll(getOutdeliveryRowsV2(iResultSet.getInt("id")));
        return iOutdelivery;
    }
    ...
    public List<SSIndelivery> getIndeliveries() {
        ...
        if (!ensureSchemaV2ForDomain("Indelivery")) {
            return iIndeliveries;
        }
        try {
            Integer iMax = -1;
    ...
    public Optional<SSIndelivery> getIndelivery(SSIndelivery pIndelivery) {
        ...
        try {
            ...
            if (iResultSet.next()) {
                SSIndelivery iIndelivery = mapIndeliveryV2(iResultSet);
    ...
    public List<SSOutdelivery> getOutdeliveries() {
        ...
        if (!ensureSchemaV2ForDomain("Outdelivery")) {
            return iOutdeliveries;
        }
        try {
            Integer iMax = -1;
            ...
            while (true) {
                ...
                while (iResultSet.next()) {
                    iMax = iResultSet.getInt(1);
                    iOutdeliveries.add(mapOutdeliveryV2(iResultSet));
                    i++;
                }
                if (i != 1024) {
                    break;
    ...
    public Optional<SSOutdelivery> getOutdelivery(SSOutdelivery pOutdelivery) {
        ...
        try {
            ...
            if (iResultSet.next()) {
                SSOutdelivery iOutdelivery = mapOutdeliveryV2(iResultSet);
    ...
    public void addOutdelivery(SSOutdelivery iOutdelivery) {
        ...
        try {
            ...
            Integer iOutdeliveryId = null;
            iStatement = iConnection.prepareStatement(
                    "INSERT INTO tbl_outdelivery(number,companyid,vdate,itext) VALUES(?,?,?,?)",
                    Statement.RETURN_GENERATED_KEYS);
            iStatement.setObject(1, iOutdelivery.getNumber());
            iStatement.setObject(2, iCurrentCompany.getId());
            bindLocalDateV2(iStatement, 3, iOutdelivery.getLocalDate());
            iStatement.setObject(4, iOutdelivery.getText());
            iStatement.executeUpdate();

            try (ResultSet iKeys = iStatement.getGeneratedKeys()) {
                if (iKeys.next()) {
                    iOutdeliveryId = iKeys.getInt(1);
                }
            }
            if (iOutdeliveryId == null) {
                iOutdeliveryId = getOutdeliveryIdV2(iOutdelivery.getNumber(), iCurrentCompany.getId());
            }
            if (iOutdeliveryId != null) {
                replaceOutdeliveryRowsV2(iOutdeliveryId, iOutdelivery);
            }
    ...
    public void updateOutdelivery(SSOutdelivery iOutdelivery) {
        ...
        try {
            ...
            bindLocalDateV2(iStatement, 1, iOutdelivery.getLocalDate());
            iStatement.setObject(2, iOutdelivery.getText());
            iStatement.setObject(3, iOutdelivery.getNumber());
            iStatement.setObject(4, iCurrentCompany.getId());
            iStatement.executeUpdate();
    ...
    public void deleteOutdelivery(SSOutdelivery iOutdelivery) {
        ...
        try {
            Integer iOutdeliveryId = getOutdeliveryIdV2(iOutdelivery.getNumber(), iCurrentCompany.getId());
            if (iOutdeliveryId != null) {
                PreparedStatement iDeleteRows = iConnection.prepareStatement(
                        "DELETE FROM tbl_outdelivery_row WHERE outdelivery_id=?");
                iDeleteRows.setObject(1, iOutdeliveryId);
                iDeleteRows.executeUpdate();
                iDeleteRows.close();
            }
